...
public class DaoSimple implements Dao {
    ...
    private String TABLE_NAME6="Mac_Key";//根密钥列表
    ...
    public void delete(String s1) {
        ...
        db.execSQL("delete from "+TABLE_NAME6+" where flag=?",new String[]{s1});
        db.close();
    }
    ...
    @Override
    public void macAdd(MacKey.Bean mac) {
        db=helper.getWritableDatabase();
        ContentValues values=new ContentValues();
        values.put("version",mac.getVersion());
        values.put("appid",mac.getAppid());
        values.put("key_num",mac.getKey_num());
        values.put("mac_key_list",mac.getMac_key_list());
        values.put("result_code",mac.getResult_code());
        values.put("err_code",mac.getErr_code());
        values.put("err_code_des",mac.getErr_code_des());
        values.put("optiontime",mac.getOptiontime());
        db.insert(TABLE_NAME6,null,values);
        db.close();
    }

    @Override
    public void macUpd(String data1, String data2) {
        db=helper.getWritableDatabase();
        db.execSQL("update "+TABLE_NAME6+" set flag=? where flag=?",new String[]{data1,data2});
        db.close();
    }

    @Override
    public MacKey.Bean macSel(String roomno) {
        return null;
    }

    @Override
    public ArrayList<MacKey.Bean> macSelAll() {
        db=helper.getWritableDatabase();
        ArrayList<MacKey.Bean> list= new ArrayList<>();
        Cursor c=null;
        try{
            c=db.rawQuery("select * from "+ TABLE_NAME6,null);
            for (int i = 0; i <c.getCount() ; i++) {
                c.moveToNext();
                MacKey.Bean mac =new MacKey().new Bean();
                mac.setVersion(c.getString(c.getColumnIndex("version")));
                mac.setAppid(c.getString(c.getColumnIndex("appid")));
                mac.setKey_num(c.getString(c.getColumnIndex("key_num")));
                mac.setMac_key_list(c.getString(c.getColumnIndex("mac_key_list")));
                mac.setErr_code(c.getString(c.getColumnIndex("err_code")));
                mac.setErr_code_des(c.getString(c.getColumnIndex("err_code_des")));
                mac.setResult_code(c.getString(c.getColumnIndex("result_code")));
                mac.setOptiontime(c.getString(c.getColumnIndex("optiontime")));
                list.add(mac);
            }
